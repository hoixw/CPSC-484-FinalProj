document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("map").addEventListener("load", function () {
        ...
        function addTableOutline(tableNum) {
            const tableOutline = mapObject.querySelector(
                `[data-outline="${tableNum}"]`,
            );
            if (tableOutline) {
                ...
                if (tableInfo) {
                    ...
                    if (seat) {
                        ...
                        while (colorSelectionResponse !== "yes") {
                            ...
                            hideModal();
                            colorSelectionResponse = await showConfirmation(
                                "Color:",
                                responseFromColorSelection.charAt(0).toUpperCase() +
                                    responseFromColorSelection.slice(1),
                            );
        ...
        function updateSVGMap() {
            ...
            for (const tableID in tableMap) {
                ...
                if (tableData.vibe) {
                    const teapot = mapObject.querySelector(
                        `[data-table="${tableID}"][data-vibe="teapot"]`,
                    );
                    const timer = mapObject.querySelector(
                        `[data-table="${tableID}"][data-vibe="timer"]`,
                    );
                ...
                for (let seat of tableData.seats) {
                    if (seat.isOccupied) {
                        const seatElement = mapObject.querySelector(
                            `[data-table="${tableID}"][data-seat="${seat.seat}"]`,
                        );
                        if (seatElement) {
